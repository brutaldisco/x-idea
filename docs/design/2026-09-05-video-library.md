...
- 動画を見るだけなら **X へ遷移**（Reader の未保存サムネイルタップ）。
...
4. 完了したら `complete` API で `ready` 化（レスポンスを検査し、最大 3 回リトライ）。総サイズが分かっているのに受信が足りない場合は完了にせず `failed` にし、**途中の mp4 は削除**する（再試行は最初から）。取消も同様に途中ファイルを消す。Videos を開いたとき／「途中ファイルを削除」で、`ready` / `queued` / `downloading` 以外の孤立 mp4 を掃除する。**完了した動画は残りのダウンロードを待たず、その時点でライブラリに表示する**。中断・停止した項目は `action: "requeue"` で `queued` に戻す。進捗のある `downloading` として残った項目（別セッションの取り残し）は「中断」表示にし、「再開」ボタンと「すべて開始」の対象に含める。
...
1. **ダウンロードキュー**：件数（`N / 15`）＋「すべて開始」（実行中は **「停止」**）＋「途中ファイルを削除」。チェックで対象を選び「選んだ N 件を開始」、各行に「この動画だけ」。各アイテムはサムネイル（右下に再生時間）・投稿抜粋・`@username`・状態（日本語。ダウンロード中は `42%` など）・進捗バーと受信量・取消。取れるときはサイズと画質（`480p` など）を操作の左に出す。`failed` は理由と「再試行」（途中ファイルは消して最初から）。進捗のない `downloading` は「中断」＋「再開」。完了した動画は即座にライブラリへ出る。未リンク時は Settings への案内。
2. **ライブラリ**：フォルダチップ（`すべて / 未分類 / {フォルダ}… / ＋新規フォルダ`）。グリッドカードはサムネイル（WebP blob）・再生時間バッジ・投稿抜粋・保存日・画質（取れるとき）・サイズ。操作メニューに「フォルダ移動」「削除」（確認で mp4 も消えると明示し、保存フォルダのファイルも消す）「X で開く」「Source を開く」。
3. **プレーヤー**：カードをタップで **黒ベースのモーダル**（ライト／ダークどちらでも `bg-black`。テーマの paper/ink は使わない）。`<video controls playsInline>` に `handle.getFile()` → `URL.createObjectURL()` を渡す（閉じたら `revokeObjectURL`）。シーク・音量はブラウザ標準。映像面のクリックで再生／一時停止（コントロールバーは除く。ダブルクリックは全画面）。ポインタが 1 秒動かなければ上下 UI とネイティブコントロールをゆっくりフェードアウトし、動かすと再表示する。全画面はプレーヤー枠（シェル）に対して行い、動画が終わって次へ進んでも、左右キーで前後しても維持する。左右キーはシークせず前後の動画へ（ネイティブのシークバーにフォーカスがあっても同じ。再生位置はバーをドラッグして動かす）。

### 5.2 Reader（SC-06）の変更

- 動画タイルは **未保存サムネイルのタップで X を開く**（実装済み）に加えて **「保存する」** ボタンを出す（挙動はキュー投入のまま）。
...
| Source 削除 | `media_assets` カスケードで `video_downloads` 行も消える。確認ダイアログで **画像と手元の mp4 も消える** と明示し、ブラウザの File System Access API で保存フォルダのファイルも削除する。フォルダ未リンク／権限なしのときだけ Finder 案内 |
