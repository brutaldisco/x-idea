...
- 失敗しても同期全体は止めない。
- 保存済みも同じ判定を適用する。同期／backfill のあと、未確認の Source を保存順に最大 100 件 `GET /2/tweets?ids=` で見る。`x_account.gone_sweep_cursor` で続きから再開する。一周したら同じ実行で先頭から再開する。課金は Post read（$0.005/件）。スレッド月次上限には入れない。
