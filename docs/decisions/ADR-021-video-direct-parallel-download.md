...
4. **プロキシ経路の本文ストール検知**: チャンクの本文読み込みにも
   ...
   （今回の 0 B/s 固着の主因）。
5. **URL 解決・サイズ探知のタイムアウト**（同日追記）:
   `GET /api/media/[id]/url` とブラウザ側 HEAD / Range 0-0 プローブに
   30 秒（`VIDEO_URL_RESOLVE_MS`）のタイムアウトを付ける。最初の
   1 バイトが来るまでウォッチドッグはまだ動かないため、ここで
   固まるとリースが切れるまで何も起きなかった。URL 解決の
   タイムアウトはプロキシ経路へ進み、プローブのタイムアウトは
   「サイズ不明」として逐次経路へ進む。ユーザーの停止シグナルは
   従来どおり中断として上に投げる。
