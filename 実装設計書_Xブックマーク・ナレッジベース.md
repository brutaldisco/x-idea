...
- **カード**：投稿者、**投稿日**（`posted_at`、なければ `bookmarked_at` / `saved_at`）、要約、サムネ。動画サムネは保存状態でバッジを変える（未保存「動画」／キュー中「保存中」／`video_downloads.status='ready'`「保存済」）。**3 点メニュー**から「X で開く」と **削除**（DB 行＋ローカル画像/動画ファイル。`dismissed_bookmarks` に残し同期では戻さない。`bookmark.write` があれば X のブックマークも外す。ADR-013）。
