...
> **v3.3 の要点（v3.2 からの変更）**
> 1. **アカウントコンテキスト（ビュー切替）を追加**（ADR-003、ADR-014）。指向性の異なるアカウント同士を混ぜないため、Today / Inbox / Library / Ask は **選択中の 1 アカウントだけ** にスコープする。「すべて」は持たない。Cookie 未設定時は Settings の既定アカウント、それも無ければ先頭。`min-[48rem]` 以上では画面左下に現在の `@name` を出し（タブバーには入れない）、タップで「アカウントを切り替える」と候補を開く。モバイルでは左下に出さず、Settings ページ上部の「表示するアカウント」で切り替える。HttpOnly Cookie `x_ctx`。`user_id` は追加しない。
...
下部タブは6項目固定：**Today / Inbox / Library / Videos / Ask / Settings**（v3.5 で Videos を追加）。Atlas は Library の表示切替、Briefing・Echo は Today からの遷移。タブ画面と Capture のページタイトルは英語のみ（小さい英語ラベル＋日本語見出しの二重表示はしない）。Today の見出しは「Today」。
...
- **アカウントコンテキスト**（v3.3）：`min-[48rem]` 以上は画面左下に現在の `@name` を出す（タブバーの外）。タップで「アカウントを切り替える」と候補一覧。モバイルでは出さない（切替は Settings ページ上部の「表示するアカウント」）。Today の同期ピル・Inbox 件数・最近の Source は選択中アカウントだけに絞る。
...
- **アカウントコンテキスト**（v3.3）：一覧は選択中コンテキストに絞る。空状態は「{コンテキスト}の Inbox は空です」。
...
- **並び**：既定は X のブックマーク一覧と同じく、ブックマークした日の新しい順。X API はブックマーク時刻を返さないため、初回観測時刻 `saved_at` で近似する（R-10 / P-12 / ADR-019）。Library / Inbox は `?sort=` で **新しい順 / 古い順 / 動画の保存済み** を切り替えられる。動画の保存済みは `video_downloads.status='ready'` を先頭にし、同じグループ内はブックマーク順。重要度・関連度（Lens 時）は後続。古い `?sort=saved_asc` は古い順。
- **カード**：カテゴリ／情報タイプの小さいバッジ（Settings の accent 色。タップでドロップダウンから変更。未設定は「Category」「Type」。メニューは画面内に収め、下端がタブバーに隠れるときは上に開く。スクロール中はカードに追従する）、**投稿日**（`posted_at`、なければ `bookmarked_at` / `saved_at`）、要約、サムネ。**投稿に動画があるときは、投稿写真・記事 og:image を含むあらゆる静止画より動画サムネを出す**（Reader ヒーローと同じ）。**画像が無い投稿は Library 一覧のイメージエリア中央にだけ投稿者アバター**（取れなければ名前の頭文字。Reader には出さない）。投稿者ハンドルは出さない。動画サムネは右下に再生時間（`media_assets.duration_ms`）。保存済みは緑のバレット、キュー中は黄のバレットを時間の左に付ける。未保存の「動画」ラベルは出さない。Library カードのサムネに出す。Reader ギャラリーはタイル下部だけ（サムネ上は重ねない）。`ready` の動画サムネはタップでアプリ内再生（**Videos と同じ全画面プレーヤー**）。**3 点メニュー**は、投稿に未保存の動画があれば先頭が **動画を保存する**（確認せずその投稿の全動画をキューへ。すでに `ready` / `queued` / `downloading` の分は飛ばす。上限 15 件。ページは移らない）、続いて「X で開く」、**削除**（DB 行＋ローカル画像/動画ファイル。`dismissed_bookmarks` に残し同期では戻さない。`bookmark.write` があれば X のブックマークも外す。ADR-013）。
...
- **ヒーロー**：投稿者アバター・名前・日時。右にカテゴリ／情報タイプ／状態／種類のドロップダウン（変更可。未設定は項目名。カテゴリと情報タイプは Settings の accent 色）。その右に 3 点メニュー（動画を保存する／X で開く／削除）。「X で開く」は 3 点メニューと原文カード。サムネイルは一覧からの `<ViewTransition name="source-{id}">` 共有要素。写真はフル画像（`/api/media/{id}`、`name=orig`／ローカル保存）。**投稿に動画（`video` / `animated_gif`）があるときはヒーローも Library カードも動画にする**（投稿写真・記事画像より優先）。**記事ブックマークで投稿に動画も写真も無いとき**は記事の `og:image`（`twitter:image` / `og:image:secure_url` / JSON-LD / 本文先頭画像 / X の URL カード画像も可）を WebP にして `media_blobs` に保存し、ヒーローと Library カードに出す。X ネイティブ記事は API の `cover_media` / `media_entities` を表紙にする（値が media_key のときは `article.cover_media` / `article.media_entities` 展開で `includes.media` の URL に解決する）。URL は `https://x.com/i/article/{id}` に揃える（`http://` と別行にしない）。`thumbnail_url` が空でも、API をまだ見ていなければ取り直す（誤った `no-cover` は外す）。既存の取得済み記事も Reader 表示とバックフィルで同じ。投稿に動画も写真も記事表紙も無いときはヒーロー画像は出さない。ヒーローは切り抜き表示し、タップでフルスクリーン拡大（原文ギャラリーと同じ）。
- **セグメント**：`原文 | 記事 | 要約`（記事がなければ 2 つ）。単一スクロールで、セグメントはアンカージャンプ。バーは不透明（`bg-paper`）。
- **原文**：全文、引用投稿は入れ子カード、メディアはギャラリー（**ローカル保存を優先表示**（ADR-005）、画像タップでフルスクリーン、**OCR テキストを画像下に折り畳み表示**（P2））、**セルフスレッドは Reader で対象ごとに手動取得**し、取得後は折りたたみ（件数つき、既定は展開）。未取得時はボタン、トグル OFF 時は案内。未保存の動画はアプリ内再生せず **サムネイルタップで X を開く＋サイズ／画質（取れるとき）＋「保存する」**（ダウンロードキューへ投入、14.6 / SC-15）。`video_downloads.status='ready'` の動画はサムネタップでアプリ内再生（**Videos と同じ全画面プレーヤー**。Library カードと同じ）。動画サムネ右下に再生時間（`media_assets.duration_ms`、X 取り込み時）。**保存済**は共通バッジ（緑アウトライン・紙色フィル・緑文字）。**キュー**（`queued` / `downloading`）は共通バッジ（黄アウトライン・紙色フィル・黄文字）。Reader ギャラリーの保存／キューバッジはタイル下部だけに出し、サムネ上には重ねない。**Chrome 翻訳**（ADR-006）：原文に `lang` + `translate=yes`、日本語 UI は `translate=no`。本文側に「日本語に翻訳」（Chrome Translator API、端末内）と「原文を選択」（右クリック翻訳の起点）。翻訳手順の常時説明と失敗時の案内文は出さない。ワンクリックできないときは原文を選択するだけ。ボタンはかな優勢の日本語本文では出さず、**漢字だけの中国語など他言語では出す**。記事は投稿 `lang` を使わず、クリック時に Language Detector が本文を見る。X の自動翻訳文は API に無い。表示は段落として組む（単一改行も段落、空行はより大きな区切り）。本文の上余白は 2rem（記事と同じ）。Chrome 翻訳結果も同じ組み。Translator が改行を消した場合は句点で分ける。**「日本語に翻訳」成功時は対訳を `chrome_translations` に保存**し、再オープン時に `source_hash` が一致すれば対訳ペインを最初から出す（原文カラム・AI 列・FTS には書かない）。
- **記事**：外部リンクだけにせず、取得できた本文を **アプリ内リーダー**で表示する。タイトル（1.2rem、本文 0.9375rem より一段大きく、下に 1.5rem の余白）＋本文（上に 2rem、sanitize 済み HTML 優先、`img` を許可）＋補助の「元の記事を開く」。過去に `img` を落とした HTML は、空の画像リンクを `<img>` に戻す。戻せない分は本文を再取得する（1 記事 1 回、tick / Reader の `after`、LIMIT あり）。本文は `.reader-body.article-body`（0.9375rem / 1.85 / `letter-spacing: 0.06em` / `max-width: 40em`、カード内中央）。PC のカード左右パディングは `2rem`。プレーンテキストは行ごとに段落化。長い一行は句点（和文）と `.!?`（英文）でも段落化する。HTML のプレーンな `<p>` も同じ組み（リンク・画像付きの段落は触らない）。見出し・リスト・引用の余白は本文より大きく取る。`fetch_scope` バッジ（全文／一部／概要のみ／失敗）。既存 Source は Reader 表示時に URL を拾ってバックフィルする。モバイルはページ `0.5rem`＋カード `0.5rem` に揃えて本文幅を広げる（原文・要約カードも同じ。ADR-022）。原文カラムは表示の組みだけ変え、DB は書き換えない。
...
- **表示するアカウント（ページ上部）**：モバイルの切替入口。大きなタップ面で 1 件選び、`x_ctx` を更新する。Today / Inbox / Library / Ask と、このページの連携・分類も同じ選択。広い画面の左下切替と同じ Cookie。
- **使用量メーター**：なくなったら追加する運用。残量を大きく出し、日次バーとアカウント別バーを添える。
...
- **アカウント**：1 枚のカードにまとめる。見出しは選んだ `@name`（「このアカウントの設定」）。同じ枠内に X 連携と分類を出す。同時に複数アカウントの設定は表示しない。切替はページ上部の「表示するアカウント」（同じ `x_ctx`）。カード内のチップでも選べる。「アカウントを追加」もここ（最大 3）。
...
- **ダウンロードキュー**：`N / 15` 件表示＋「すべて開始」。チェックで対象を選び「選んだ N 件を開始」、各行の「この動画だけ」でも実行できる。実行中は **「停止」ボタン** で全体を中断できる（押した瞬間に「停止中」と出し進捗表示を止める。未書き込みチャンクは捨て、ファイルの `close` が終わるまで開始は受けない。中断分は `queued` に戻す。**手動停止は自動では再始動しない**。続きは「すべて開始」または「この動画だけ」。`requeue` はすでに `queued` でも成功し、停止直後の 409 で再開できなくならないようにする）。**失敗しても途中ファイルと進捗（IndexedDB）は残し、「途中から再開」で続きから取り直せる**（2026-09-13 改定。それ以前は失敗時に破棄していた）。**1 本の失敗や回線断で他のダウンロードは止めない**（ADR-023：動画どうしは最大 2 本並行、オフライン検知で全体を abort しない、`failed` も「すべて開始」の対象に含める）。取消・開けない途中ファイルは保存フォルダから削除する。キューに「途中ファイルを削除」があり（失敗分も明示的に消せる）、Videos を開いたときも孤立した mp4（取消・記録なし）を掃除する（`failed` は再試行用に残す）。保存済みを開いて途中ファイルだった場合も削除し、キューへ戻す。各アイテムはサムネイル（右下に再生時間）・投稿抜粋・`@username`・状態（日本語。ダウンロード中はパーセントと直近数秒の転送速度）・進捗バーと受信量（`4.6 MB / 38.7 MB`。総サイズが無い間は概算を「約」で分母にする）と速度（`12.3 MB/s`。大きなファイルでパーセントが動かなくても進行が分かる）・取消。取れるときはサイズと画質（`480p` など。最高 `bit_rate` の mp4。未保存は `bit_rate × 時間` の概算）を操作の左に出す。`failed` は理由と「途中から再開」（進捗を残して続きから取り直す）。ダウンロード中は 8 秒ごとにサーバーへ **ハートビート**（`POST /api/videos/queue/[id] {action:"progress"}`）を送り、`video_downloads.last_progress_at` / `progress_bytes` / `progress_total` に最終生存時刻と停止位置を残す（ADR-025）。このタブで進捗のない `downloading` は、ハートビートが 90 秒以内なら **「別のタブで実行中です」**（`start` はサーバー側のリースで 409 に排他される）、途絶えていれば **中断** と表示し、「再開」ボタンと「すべて開始」の対象に含める。リース切れの中断分は Videos を開いたとき（フォルダの書き込み許可済みなら）**自動で途中から再開**する。`queued`（手動停止を含む）と `failed` は自動では動かさない。停止位置は「1.2 GB まで保存済み」のように表示する。**完了した動画は残りを待たず即座にライブラリへ出す**。実行は **CDN 直接・4 並列の Range 取得**（ADR-021。失敗時は CDN 逐次、最後にプロキシ）で、**30 秒無応答のチャンクは切断してリトライ**する（14.6）。書き込みは `createWritable` を 1 回だけ開いて使い回し、両経路が 5xx・回線断で失敗したときは CDN URL を取り直して 1 回だけやり直す（ADR-026）。総サイズが分かるまでパーセントは出さない。ダウンロード中は直近数秒の平均速度（`12.3 MB/s`。観測前は「計測中」、停止すると `0 B/s`）を状態と受信量の横に出し、大きなファイルでパーセントや GB 表示が動かなくても進行が分かるようにする。
...
- **プレーヤー**：カードタップで黒ベースの全画面モーダル（ライト／ダークどちらでも黒。テーマトークンは使わない）。上部のアカウント・本文は暗めのグレー（`neutral-600`）。閉じるは × アイコン。右下の繰り返しは「この動画をくり返す」と「フォルダ内を順に再生」の 2 つ。どちらも押していなければくり返さない。同じボタンをもう一度押すとオフ。オンは白反転せず薄いハイライト。ホバーで説明チップを出す。`<video controls playsInline>` に object URL を渡す。**映像面（コントロールバー以外）のクリックで再生／一時停止**。ダブルクリックと枠のボタンは全画面。スペースキーでも再生／停止。ポインタが 1 秒動かなければ上下の独自 UI とネイティブコントロールを 0.7 秒でフェードアウトし、動かすと再表示する。全画面はプレーヤー枠に対して行い、終了や左右キーで次／前へ移っても維持する。左右キーはシークせず前後の動画へ（ネイティブのシークバーにフォーカスがあっても同じ。再生位置はバーをドラッグして動かす）。ネイティブの全画面ボタンは使わず、枠の全画面に寄せる。
...
- Reader 原文は 1.05rem（16.8px）/ 1.85、幅は `max-width: 40em`（全角 40 字）。記事本文は 0.9375rem（15px）/ 1.85 / `letter-spacing: 0.06em`。記事タイトルは 1.2rem。AI 要約は `text-sm` ＋同じ幅・行間。日本語は `font-feature-settings: "palt"`, `text-wrap: pretty`, `line-break: strict`, `hanging-punctuation: allow-end`。
...
- コントラスト AA（本文リンクを含む 4.5:1）、フォーカスリング可視、スワイプ操作には必ずボタン代替。
...
- **動画本体は自動保存しない**。未保存の Reader 動画はサムネイルタップで X を開く。残したい動画だけ「保存する」でキュー（`video_downloads`、**queued は最大 15 件**）に入れ、**Videos タブ（SC-15）から手動実行**する。保存済み（`ready`）は Library / Reader のサムネタップでローカルファイルを再生できる（Videos と同じ全画面プレーヤー）。
...
- 画質は `variants` の **最大 `bit_rate` の mp4**（最高解像度。字幕の可読性対策）。転送は **CDN 直接・4 並列の Range 取得**（ADR-021：`GET /api/media/[id]/url` で URL と総サイズ `bytes` を解決し、ブラウザから video.twimg.com へ `referrerPolicy: no-referrer` で直接取得。総サイズはサーバー側の HEAD / Range 0-0。クライアント HEAD 成功は必須にしない。チャンクは 1〜8MB。video.twimg.com はコネクション単位でスロットルするため逐次プロキシより約 40 倍速い）。並列に失敗／総サイズ不明のときは **CDN 逐次 Range** を試し、それも失敗したときだけ従来のプロキシ逐次経路（`GET /api/media/[id]/file`、`maxDuration = 300`）へフォールバックする。ローカルファイルが無い再生と Safari の「ファイルを保存」は `GET /api/media/[id]/url?redirect=1`（302 のみ）で CDN へ飛ばし、本体バイトは Vercel を通さない。レジュームは IndexedDB に **書き込み済み位置だけ** を記録する（表示用の受信量と分離）。**30 秒間 1 バイトも受信できないチャンクは切断してリトライ**（指数バックオフ。5xx はサーバー側の回復を待って最大 30 秒間隔）し、無音ストールでキュー全体が止まらないようにする。スタール検知はヘッダ到達後の本文読み込みにも効かせる。並列取得の先読みは書き込み位置から **32MB までに制限**する（背圧。1 レーンの停滞で未書き込みチャンクがメモリに溜まり続けるのを防ぐ）。File System Access の write/seek/close/getFile には **60 秒**、`createWritable` には **180 秒**（大きな途中ファイルは既存サイズに応じて最大 15 分）のタイムアウトを付ける。再開位置の最初の進捗ではスタール監視を武装せず、keepExistingData の全コピー中に 75 秒で切って開き直しを繰り返さない（2026-09-20 追記）。固まった呼び出しは失敗として落とす。スタール尽きは AbortError ではなく専用エラー（「応答なし」）に変換し、**ユーザーの停止シグナルだけを中断として扱う**（2026-09-20 改定。従来はスタール尽きが「停止」と誤判定されて failed に落ちず、フォールバックも効かなかった）。`/api/media/[id]/url` の取得とブラウザ側のサイズ探知（HEAD / Range 0-0）にも **30 秒**のタイムアウトを付ける（同日追記。最初の 1 バイトが来るまでウォッチドッグはまだ動かないため、ここで固まるとリース切れまで何も起きなかった。タイムアウト時はプロキシ経路／「サイズ不明」の逐次経路へ進む）。ファイルへの書き込みは **プロキシ逐次でも `createWritable` を 1 回だけ開いて使い回す**（ADR-026。`keepExistingData: true` の開き直しは既存内容の全コピーが走るため、チャンクごとに開くとファイルが大きいほど急激に遅くなる）。**256MB 以上の途中ファイルは残りを `{name}.mp4.part` へ先に取り、揃ってから本体へ 1 回だけ結合する**（2026-09-20 追記。4GB 超の全コピー中に CDN URL が切れて「開いています」と応答なしを繰り返すのを避ける。結合中は「保存ファイルに結合しています…」）。**`.part` の残り取得は、失敗しても URL を取り直して残りの続きから何度でも取り直す**（`.part` のサイズを再開位置に使う。同日追記。1 回だけにすると、また同じ全コピーから始まる）。両経路がリトライ可能なエラー（5xx・回線断）で失敗したときは、**CDN URL を取り直して CDN 直接から 1 回だけやり直す**（ADR-026。`refreshDirectUrl` が `/api/media/[id]/url` を再取得する）。実行中は **「停止」ボタン**で全体を中断できる（押した瞬間に「停止中」。未書き込みチャンクの flush はせず、`close` 完了まで開始は受けない）。中断分は `queued` に戻す。**手動停止は自動では再始動しない**（仕様）。続きは「すべて開始」または「この動画だけ」。`requeue` は `queued` でも成功（冪等）。再開位置は IndexedDB と実ファイルサイズの小さい方（進捗が消えてもファイルがあればそこから）。完了登録（`POST /api/videos/queue/[id]/complete`）はレスポンスを検査して最大 3 回リトライし、**完了した動画から即座にライブラリへ表示**する。進捗（IndexedDB）は完了登録が確認できてから消す（登録だけ失敗したときの取り直しを防ぐ）。ダウンロード中は **ハートビート**で `last_progress_at` と停止位置をサーバーに残す（ADR-025 改定：**受信バイトが増えたときだけ送る**。従来は無条件に送っていたため、書き込み側が固まったダウンロードがリースを持ち続け「別のタブで実行中です」のまま再開不能になった）。`start` はリースが切れた（90 秒無応答の）`downloading` だけを取れる（生きているものは 409）。**同一ブラウザでは Web Locks で排他し、再読み込みしたタブがロックを取れたら 90 秒待たずに引き継ぐ**（2026-09-20 追記。死んだタブのリースで「別のタブで実行中です」になるのを防ぐ）。バイトが **30 秒**増えなければ UI に「応答なし。再接続しています…」を出し、**75 秒**で（リース切れより前に）その本だけ切断して保存済み位置から自動で取り直す（最大 3 回。尽きたら `failed` にして理由を残す）。このタブで進捗のない `downloading` はリース切れ、またはこのタブがブラウザロックを持っているなら中断とみなし、「すべて開始」や個別の「このタブで再開」に加えて **Videos を開いたときの自動再開** で続きから復帰できる（`action: "requeue"` で `downloading` → `queued`）。ダウンロード中の UI は直近数秒の平均速度（`12.3 MB/s`。観測前は「計測中」、停止すると `0 B/s`）を状態と受信量の横に出し、大きなファイルでパーセントや GB 表示が動かなくても進行が分かるようにする。**CDN が 404（元のページが X 上で消えた）で失敗した動画**は、キュー行に「元のページが見つかりません。削除しますか？」を出し、「途中から再開」の代わりに **「投稿を削除する」**を置く（2026-09-20 追加。404 はリトライしても成功しないため）。削除は `DELETE /api/sources/[id]` で投稿自体を消し（権限があれば X のブックマークも外す）、途中ファイルと IndexedDB の進捗も消す。投稿を残したいときは従来どおり「取消」でキューから外すだけにできる。
...
- 本文・投稿の `t.co` は `article.entities.urls` / `note_tweet.entities.urls` / `tweet.entities.urls` の `unwound_url` または `expanded_url` に置き換えて保存・表示する（X アプリと同じ外部 URL。省略形の `display_url` は使わない。ADR-024）。既存の t.co 本文は Reader 表示・tick の保存済みエンティティ展開・X Article 再ハイドレートで同じ展開をする。X Article 本文中の外部 URL は別記事として取得しない。
...
CREATE TABLE video_downloads (                       -- ADR-007: 動画ダウンロードキュー兼ライブラリ台帳
  ...
  downloaded_at TEXT,
  last_progress_at TEXT,                           -- ADR-025: ハートビート最終受信（リース。90 秒で失効）
  progress_bytes INTEGER,                          -- ADR-025: 停止位置の記録（受信済みバイト）
  progress_total INTEGER                           -- ADR-025: 停止位置の記録（総サイズ）
);
...
- 一覧はページネーション（既定 `saved_at DESC, id ASC`、`limit` 既定 60、最大 100、`page` は OFFSET）。**全件 SELECT 禁止**。同期は X と同じく新しいブックマークから入れるので、同秒のタイブレークは `id ASC`（ADR-019）。
...
| T-101c | アカウントコンテキスト切替（v3.3、ADR-003 / ADR-014）：`x_ctx` Cookie、`min-[48rem]` 以上は画面左下に現在アカウント（タブバー外、モバイルは非表示）、タップで切替メニュー、Today/Inbox/Library/Ask のスコープリング、Settings ページ上部で表示アカウント切替、Settings で既定アカウント（「このアカウントの設定」とは別カード） | `src/server/x/context.ts`, `src/components/AccountSwitcher.tsx`, `SettingsViewingAccountCard`, Settings、`settings.default_x_account_id` | T-101b | 広い画面では左下の `@name` をタップすると「アカウントを切り替える」と候補が出る。モバイルでは左下に出ない。Settings ページ上部で切替できる。切替で Today / Inbox が変わる。既定を変えても表示中は動かない。Cookie 未設定時は Settings の既定、未設定なら先頭 |
...
| T-223 | 記事ヒーロー画像（`og:image` / X 表紙 → WebP / `media_blobs`）。本文 `img` を残す。投稿に動画も写真も無いとき一覧と Reader に出す。既存記事もバックフィル | `article-thumb.ts`, `article-html.ts`, `articleFetch.ts` | T-207, T-607 | 記事ブックマークでヒーローと本文画像が出る。X の動画または写真がある投稿は上書きしない |
...
| P-27 | アカウントコンテキスト | Cookie → Settings の既定 → 先頭。広い画面は左下で切替（タブバー外）、モバイルは Settings ページ上部。「すべて」なし（v3.3 / ADR-014） | 指向性の異なるアカウントを混ぜない |
