...
- **カード**：カテゴリ／情報タイプの小さいバッジ（Settings の accent 色。タップでドロップダウンから変更。未設定は「Category」「Type」）、**投稿日**（`posted_at`、なければ `bookmarked_at` / `saved_at`）、要約、サムネ。投稿者ハンドルは出さない。動画サムネは右下に再生時間（`media_assets.duration_ms`）。保存済みは緑のバレット、キュー中は黄のバレットを時間の左に付ける。未保存の「動画」ラベルは出さない。Library カードのサムネに出す。Reader ギャラリーはタイル下部だけ（サムネ上は重ねない）。`ready` の動画サムネはタップでアプリ内再生（**Videos と同じ全画面プレーヤー**）。**3 点メニュー**は、投稿に未保存の動画があれば先頭が **動画を保存する**（確認せずその投稿の全動画をキューへ。すでに `ready` / `queued` / `downloading` の分は飛ばす。上限 15 件。ページは移らない）、続いて「X で開く」、**削除**（DB 行＋ローカル画像/動画ファイル。`dismissed_bookmarks` に残し同期では戻さない。`bookmark.write` があれば X のブックマークも外す。ADR-013）。
