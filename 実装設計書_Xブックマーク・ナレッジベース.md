...
- **並び**：既定は X のブックマーク一覧と同じく、ブックマークした日の新しい順。X API はブックマーク時刻を返さないため、初回観測時刻 `saved_at` で近似する（R-10 / P-12 / ADR-019）。Library / Inbox は `?sort=` で **新しい順 / 古い順 / 動画の保存済み / 動画未保存** を切り替えられる。動画の保存済みは `video_downloads.status='ready'` を先頭にし、同じグループ内はブックマーク順。動画未保存は動画があり `ready` でない投稿を先頭にし、その中はカードの投稿日（`posted_at`）の新しい順にする（一括取り込みで `saved_at` が同じだとブックマーク時刻では投稿日が順に見えない）。未保存以外はブックマーク順のまま。重要度・関連度（Lens 時）は後続。古い `?sort=saved_asc` は古い順。
- **カード**：カテゴリ／情報タイプの小さいバッジ（Settings の accent 色。タップでドロップダウンから変更。未設定は「Category」「Type」。メニューは画面内に収め、下端がタブバーに隠れるときは上に開く。スクロール中はカードに追従する）、**投稿日**（`posted_at`、なければ `bookmarked_at` / `saved_at`）、要約、サムネ。**投稿に動画があるときは、投稿写真・記事 og:image を含むあらゆる静止画より動画サムネを出す**（Reader ヒーローと同じ）。**画像が無い投稿は Library 一覧のイメージエリア中央にだけ投稿者アバター**（取れなければ名前の頭文字。Reader には出さない）。投稿者ハンドルは出さない。動画サムネは右下に再生時間（`media_assets.duration_ms`）。保存済みは緑のバレット、キュー中は黄のバレットを時間の左に付ける。未保存の「動画」ラベルは出さない。Library カードのサムネに出す。保存済み動画の再生中はプレーヤーの 3 点メニュー「サムネイルを設定する」で、その秒のフレームを Library / Videos の動画サムネにする（秒数は MP4 / MOV / M4V 末尾の固定長 uuid ボックス（ADR-027。対応していない形式だけ従来の `.lvl.json`）。この端末の保存フォルダから切り出す。写真サムネは変えない）。Reader ギャラリーはタイル下部だけ（サムネ上は重ねない）。`ready` の動画サムネはタップでアプリ内再生（**Videos と同じ全画面プレーヤー**）。**3 点メニュー**は、投稿に未保存の動画があれば先頭が **動画を保存する**（確認せずその投稿の全動画をキューへ。すでに `ready` / `queued` / `downloading` の分は飛ばす。上限 15 件。ページは移らない）、続いて「X で開く」、**削除**（DB 行＋ローカル画像/動画ファイル。`dismissed_bookmarks` に残し同期では戻さない。`bookmark.write` があれば X のブックマークも外す。ADR-013）。
