...
- **戻り**：ヘッダー「← ライブラリ」は直近の Library URL（並び・フィルタ・表示）へ戻す。`/library` 固定にはしない（8.3）。Library 起点では並列ルートで一覧を残す（ADR-016）。タブへソフトナビしたときは `@reader/[...catchAll]` で Reader スロットを閉じ、一覧や Today の下に本文を残さない。同じ並びの隣へは一覧に戻らず「前の記事」「次の記事」で進む。並びは直近の Library 一覧（フィルタ・ソート込み）。一覧に無い Source やディープリンクでは出さない。TabBar の選択は Reader 中も開いた元のタブを維持する（ディープリンクは Library）。
