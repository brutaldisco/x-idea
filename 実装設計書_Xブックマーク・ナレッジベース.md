...
- **記事**：外部リンクだけにせず、取得できた本文を **アプリ内リーダー**で表示する。タイトル＋本文（sanitize 済み HTML 優先）＋補助の「元の記事を開く」。本文は `.reader-body`（1.05rem / 1.85 / `max-width: 40em`）。プレーンテキストは行ごとに段落化。HTML は見出し・段落・リスト・引用の余白を本文より大きく取る。`fetch_scope` バッジ（全文／一部／概要のみ／失敗）。既存 Source は Reader 表示時に URL を拾ってバックフィルする。モバイルは左右パディングを詰めて本文幅を広げる。
