...
- **表示**：リスト（密）／グリッド（サムネ重視、**既定**。モバイルは 2 列、`min-[48rem]` 以上は 3 列）／**Atlas**。リスト／グリッド切替でもページ幅は変えない（カードは枠内で折り返す）。フィルタの `<select>` はフォーカスリングを出さない。
