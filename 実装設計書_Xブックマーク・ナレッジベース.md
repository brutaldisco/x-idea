...
- **オムニボックス**：1つの入力。入力中は FTS 即時サジェスト（デバウンス 150ms、上位 8 件、サムネまたは投稿者アバター付きの行）。Enter でハイブリッド検索結果一覧（P1 は FTS のみ）。一覧は Library グリッドと同じ **画像付きカード**（サムネ、無いときは投稿者アバター。動画は再生時間）。「AI に聞く」ボタン（または `⌘Enter`）で RAG チャットへ。
