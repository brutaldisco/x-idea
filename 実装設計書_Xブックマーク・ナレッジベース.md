...
- **分類**：選んだアカウントのカテゴリと情報タイプ（追加・改名・削除・**ハンドルで並べ替え**）。ドラッグ中は指に追従するプレビューとプレースホルダで位置が分かる。見出しに `@ハンドル` を出す。初期値は seed カテゴリと既定の情報タイプ。Library の絞り込みと AI enrich がこの一覧の順を使う（`account_taxonomy.sort_order`、ADR-015）。
