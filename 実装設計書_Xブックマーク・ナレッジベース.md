...
- **記事**：外部リンクだけにせず、取得できた本文を **アプリ内リーダー**で表示する。タイトル（1.2rem、本文 1rem より一段大きく、下に 1.5rem の余白）＋本文（上に 2rem、sanitize 済み HTML 優先、`img` を許可）＋補助の「元の記事を開く」。過去に `img` を落とした HTML は、空の画像リンクを `<img>` に戻す。戻せない分は本文を再取得する（1 記事 1 回、tick / Reader の `after`、LIMIT あり）。本文は `.reader-body.article-body`（1rem / 1.85 / `letter-spacing: 0.03em` / `max-width: 40em`、カード内中央）。PC のカード左右パディングは `2rem`。プレーンテキストは行ごとに段落化。長い一行は句点（和文）と `.!?`（英文）でも段落化する。HTML のプレーンな `<p>` も同じ組み（リンク・画像付きの段落は触らない）。見出し・リスト・引用の余白は本文より大きく取る。`fetch_scope` バッジ（全文／一部／概要のみ／失敗）。既存 Source は Reader 表示時に URL を拾ってバックフィルする。モバイルはページ `0.5rem`＋カード `0.5rem` に揃えて本文幅を広げる（原文・要約カードも同じ。ADR-022）。原文カラムは表示の組みだけ変え、DB は書き換えない。
...
- Reader 原文は 1.05rem（16.8px）/ 1.85、幅は `max-width: 40em`（全角 40 字）。記事本文は 1rem（16px）/ 1.85 / `letter-spacing: 0.03em`。記事タイトルは 1.2rem。AI 要約は `text-sm` ＋同じ幅・行間。日本語は `font-feature-settings: "palt"`, `text-wrap: pretty`, `line-break: strict`, `hanging-punctuation: allow-end`。
